...
diagnostic <- TRUE
setwd("C:/Users/Felix/Documents/College Squash Rankings/College Squash Rankings")
# Get name of file from user and imports data from the given file
data <- read.csv("CSA mens results thru 3.1.2020.csv", header=TRUE, stringsAsFactors=FALSE)
#data <- read.csv("CSA Full Women's Results 02.24.2020.csv", header=TRUE, stringsAsFactors=FALSE)
...
# This is the K-factor, the points adjustment parameter
# Value past 4.6 takes too long to converge
K <- 1
cat("\n")
cat(paste("The point scale value is: ", K, "\n", sep=""))
...
{
  if (abs(Ratings[i,3] - Ratings[i + 1,3]) < convergenceValue)
  {
      cat(paste("'",Ratings[i,2],"'", " and ","'", Ratings[i + 1,2],"'"," are tied in ranking points. Please manually adjust", "\n", sep=""))
      cat("\n")
  }
}
cat("-----------------------------------------------\n")
...
output <- paste("output_", "CSA mens results thru 3.1.2020.csv", sep="")
#output <- paste("output_", "CSA Full Women's Results 02.24.2020.csv", sep="")
